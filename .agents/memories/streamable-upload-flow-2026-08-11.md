...
## RUST VIDEO UPLOAD IMPLEMENTATION (2026-08-12)

`StreamableClient::upload_video` implements the verified core browser flow for
both authenticated and unauthenticated client states:

1. validate and canonicalize a local video path;
2. GET `/api/v1/uploads/shortcode?size=<bytes>&version=unknown`;
3. POST `/api/v1/videos/<shortcode>/initialize` with `original_size`,
   `original_name`, `upload_source="web"`, and file-stem `title`;
4. stream the file through the existing crate-private SigV4 direct S3 PUT;
5. POST `/api/v1/transcode/<shortcode>` with `upload_source="web"`, `key`,
   `token`, `shortcode`, and `size`;
6. return public `Video` metadata from the transcode response.

Upload event tracking is informational and is intentionally absent from the Rust
flow. Do not add `/uploads/<shortcode>/track` calls to `upload_video`.

`UploadCancellationToken` enables cooperative in-flight cancellation through
`upload_video_with_cancellation`. Cancellation drops the active S3/request future
and, once a shortcode exists, sends a bodyless `POST
/api/v1/videos/<shortcode>/cancel`. `cancel_video_upload` also exposes that
server operation directly. Successful cancellation returns typed
`StreamableError::UploadCancelled` and preserves the assigned shortcode when one
exists.

Deterministic WireMock coverage asserts exact effective paths, query, request
order, JSON field names, raw file body, signed headers, absence of `x-amz-acl`,
